...
export default function CreateRoomService(props) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (props.service) {
            put(route("roomServices.update", props.service.id));
        } else {
            post(route("roomServices.store"));
        }
        setData("name", "");
    };
    return (
        <Authenticated
            ...
            header={
                <h2 className="font-semibold text-xl text-gray-800 leading-tight">
                    {props.service ? "ویرایش" : "تعریف"} سرویس اتاق{" "}
                </h2>
            }
        >
